...
const LocalOperator = () => {
  ...
  useEffect(() => {
    ...
    // Auto-start if we have an initial prompt from the home page
    if (state.initialPrompt && state.from === 'home') {
      setTimeout(() => {
        // Find the ChatInput and trigger its run mechanism manually or via state.
        // We handle this inside ChatInput by passing initialPrompt down.
      }, 500);
    }
  }, [state.sessionId]);
  ...
  return (
    <div className="flex flex-col w-full h-full bg-transparent text-white">
      <NavHeader
        ...
        onBack={handleBack}
        docUrl="https://github.com/alterhumans/macclaw"
      ></NavHeader>
